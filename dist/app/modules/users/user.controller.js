...
const registerUser = async (req, res) => {
    ...
    try {
        ...
        if (!userExists) {
            ...
            const registeredUser = await UserService.registerUser(email, password, setRole);
            res
                .status(200)
...
const loginUser = async (req, res) => {
    ...
    try {
        ...
        const isValidPassword = await UserService.ValidatePassword(password, userPassword);
        if (!isValidPassword) {
            res.status(400).send({
                message: "Invalid password, Please write correct password or try reset your password and write or save it somewhere.",
            });
            return;
        }
